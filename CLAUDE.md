...
Mudabbir is a self-hosted AI agent that runs locally and is controlled via Telegram, Discord, Slack, WhatsApp, or a web dashboard. The Python package is named `mudabbir` (the internal/legacy name), while the public-facing name is `mudabbir`. Python 3.11+ required.
...
- `DiscordAdapter` — discord.py (optional dep `Mudabbir[discord]`). Slash command `/paw` + DM/mention support. Stream buffering with edit-in-place (1.5s rate limit).
