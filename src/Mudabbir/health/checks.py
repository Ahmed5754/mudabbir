...
def check_config_exists() -> HealthCheckResult:
    """Check that ~/.Mudabbir/config.json exists."""
    from Mudabbir.config import get_config_path
...
def check_config_valid_json() -> HealthCheckResult:
    ...
    try:
        ...
        return HealthCheckResult(
            ...
            fix_hint="Fix the JSON syntax in ~/.Mudabbir/config.json or delete it to reset.",
        )
...
def check_disk_space() -> HealthCheckResult:
    """Check that ~/.Mudabbir/ isn't too large."""
    from Mudabbir.config import get_config_dir
    ...
    try:
        ...
        if total_mb > 500:
            return HealthCheckResult(
                ...
                fix_hint="Clear old sessions or audit logs in ~/.Mudabbir/",
            )
        return HealthCheckResult(
            check_id="disk_space",
            name="Disk Space",
...
def check_audit_log_writable() -> HealthCheckResult:
    ...
    if not audit_path.exists():
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                fix_hint="Check permissions on ~/.Mudabbir/",
            )
    ...
    try:
        ...
        return HealthCheckResult(
            ...
            fix_hint="Check permissions: chmod 600 ~/.Mudabbir/audit.jsonl",
        )
...
def check_memory_dir_accessible() -> HealthCheckResult:
    ...
    return HealthCheckResult(
        ...
        fix_hint="Remove the file at ~/.Mudabbir/memory and restart.",
    )
