...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install 'Mudabbir[telegram]'"
    ) from _exc

from Mudabbir.bus import (
    BaseChannelAdapter,
    Channel,
    InboundMessage,
    OutboundMessage,
...
def _looks_like_technical_leak(text: str) -> bool:
    if not text:
        return False
    ...
    if re.search(r"^\s*name\s+pyautogui", lowered) and "screenshot" in lowered:
        return True
    if '{"name": "execute"' in lowered or '"arguments"' in lowered and '"code"' in lowered:
        return True
    if re.search(r'^\s*"?\s*(name|arguments|language|code)\s*"?\s*:', lowered):
        return True
    if lowered.startswith("{") and ("execute" in lowered or "pyautogui" in lowered):
        return True
    return False
...
def _sanitize_telegram_text(text: str) -> str:
    ...
    if not raw.strip():
        return ""
    if _looks_like_technical_leak(raw):
        return ""

    cleaned_lines: list[str] = []
    for line in raw.splitlines():
        if _looks_like_technical_leak(line):
            continue
...
class _TelegramPollingNoiseFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        # Benign when another Telegram poller is active for the same token.
        if "terminated by other getupdates request" in message:
            return False
        if "error while calling `get_updates` one more time" in message and "suppressing error" in message:
            return False

        if "exception happened while polling for updates" not in message:
            return True

        exc = record.exc_info[1] if record.exc_info else None
        exc_text = str(exc or "").lower()
        if "terminated by other getupdates request" in exc_text:
            return False
        if "getaddrinfo failed" in exc_text or "httpx.connecterror" in exc_text:
            return False
        return True
...
class TelegramAdapter(BaseChannelAdapter):
    ...
    _polling_noise_filter_installed = False
    ...
    def __init__(self, token: str, allowed_user_id: int | None = None):
        ...
        self._last_user_text_by_chat: dict[str, str] = {}

    @classmethod
    def _install_polling_noise_filter(cls) -> None:
        if cls._polling_noise_filter_installed:
            return
        filt = _TelegramPollingNoiseFilter()
        for logger_name in (
        ...
        cls._polling_noise_filter_installed = True
    ...
    async def _on_start(self) -> None:
        ...
        self._install_polling_noise_filter()
    ...
    async def send(self, message: OutboundMessage) -> None:
        ...
        try:
            ...
            sanitized_content = _sanitize_telegram_text(str(message.content or ""))
            if not sanitized_content:
                return
            real_chat_id, topic_id = self._parse_chat_id(chat_id)
            send_kwargs: dict[str, Any] = {
                "chat_id": real_chat_id,
                "text": convert_markdown(sanitized_content, self.channel),
                "parse_mode": "Markdown",
            }
    ...
    async def _handle_stream_chunk(self, message: OutboundMessage) -> None:
        ...
        if not raw_content.strip():
            return
        content = _sanitize_telegram_text(raw_content)
        is_technical = _looks_like_technical_leak(raw_content)
        if not content and not is_technical:
            return
        ...
        if chat_id not in self._buffers:
            ...
            self._buffers[chat_id]["raw_text"] += raw_content
            if content:
        ...
        if now - buf["last_update"] > 1.5 and str(buf.get("text", "")).strip():  # Update every 1.5s
    ...
    async def _update_message(self, chat_id: str, message_id: int, text: str) -> None:
        try:
            safe_text = _sanitize_telegram_text(text)
            if not safe_text.strip():
                return
    ...
    async def _send_media_file(self, chat_id: str, file_path: str) -> None:
        ...
        from Mudabbir.bus.adapters import guess_media_type
    ...
    async def _handle_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        self._last_user_text_by_chat[chat_id] = text
    ...
    async def _handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if file_obj:
            try:
                from Mudabbir.bus.media import build_media_hint, get_media_downloader
        ...
        chat_id = f"{base_chat_id}:topic:{topic_id}" if topic_id else base_chat_id
        if content:
            self._last_user_text_by_chat[chat_id] = content
