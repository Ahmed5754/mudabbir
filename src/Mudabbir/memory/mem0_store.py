...
def _build_mem0_config(
    ...
    """Build a mem0 config dict from Mudabbir settings.

    Returns a dict suitable for Memory.from_config().
    """
    data_path = data_path or (Path.home() / ".Mudabbir" / "mem0_data")
...
class Mem0MemoryStore:
    ...
    async def save(self, entry: MemoryEntry) -> str:
        ...
        # Build metadata
        metadata = {
            "Mudabbir_type": entry.type.value,
            "tags": entry.tags,
    ...
    async def search(
        ...
        filters = {}
        if memory_type:
            filters["Mudabbir_type"] = memory_type.value
    ...
    def _mem0_to_entry(self, mem0_item: dict) -> MemoryEntry:
        ...
        # Parse memory type
        type_str = metadata.get("Mudabbir_type", "long_term")
        try:
        ...
        return MemoryEntry(
            ...
            metadata={
                ...
                if k not in ("Mudabbir_type", "tags", "created_at", "role")
            },
            role=role,
    ...
    async def get_memory_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            for item in results:
                mem_type = item.get("metadata", {}).get("Mudabbir_type", "unknown")
