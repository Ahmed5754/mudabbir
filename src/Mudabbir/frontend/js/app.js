...
function app() {
    ...
    const { state: featureStates, methods: featureMethods } =
        window.Mudabbir.Loader.assemble();
    ...
    return {
        ...
        async submitToken() {
            ...
            try {
                ...
                this.loginError = 'Connection failed. Is Mudabbir running?';
            } finally {
        ...
        init() {
            this.log('Mudabbir Dashboard initialized', 'info');
            ...
            if (masterToken) {
                ...
                localStorage.setItem('Mudabbir_token', masterToken);
                ...
                fetch('/api/auth/session', {
                    ...
                    if (data && data.session_token) {
                        localStorage.setItem('Mudabbir_token', data.session_token);
            ...
            window.fetch = async (url, options = {}) => {
                const storedToken = localStorage.getItem('Mudabbir_token');
                ...
                if (response.status === 401 || response.status === 403) {
                    localStorage.removeItem('Mudabbir_token');
            ...
            const hasToken = !!localStorage.getItem('Mudabbir_token');
         ...
         */
        _startApp() {
            // Desktop/mobile viewport state (used by sidebar collapse behavior)
            ...
            this.sidebarCollapsed = !!StateManager.load('sidebarCollapsed', false);

            // Wire EventBus listeners (cross-module communication)
            Mudabbir.EventBus.on('sidebar:files', (data) => this.handleSidebarFiles(data));
            Mudabbir.EventBus.on('output:files', (data) => this.handleOutputFiles(data));
        ...
        setupSocketHandlers() {
            ...
            const onConnected = () => {
                this.log('Connected to Mudabbir Engine', 'success');
        ...
        handleSettings(data) {
            ...
            if (!this.hasAnthropicKey && !localStorage.getItem('Mudabbir_setup_dismissed')) {
        ...
        /**
         * Set desktop sidebar collapsed state and persist it.
         */
        setSidebarCollapsed(collapsed) {
            this.sidebarCollapsed = !!collapsed;
            StateManager.save('sidebarCollapsed', this.sidebarCollapsed);
            this.$nextTick(() => {
                if (window.refreshIcons) window.refreshIcons();
            });
        },
