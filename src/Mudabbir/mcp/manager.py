...
class MCPManager:
    ...
    def _make_oauth_auth(config: MCPServerConfig):
        ...
        from Mudabbir.mcp.oauth_store import MCPTokenStorage
    ...
    async def start_enabled_servers(self, include_oauth: bool = False) -> None:
        """Start all enabled servers from config.

        By default, OAuth servers are skipped during startup to avoid
        blocking application boot while waiting for interactive auth callbacks.
        They can still be started manually from the dashboard.
        Context7 is also skipped during startup to avoid noisy runtime warnings;
        users can still start it manually from MCP panel.
        """
        configs = load_mcp_config()
        for config in configs:
            if not config.enabled:
                continue
            if config.name.lower() == "context7":
                logger.info(
                    "Skipping MCP server '%s' during startup; start it from MCP panel when needed",
                    config.name,
                )
                continue
            if config.oauth and not include_oauth:
                logger.info(
                    "Skipping OAuth MCP server '%s' during startup; start it from MCP panel when needed",
                    config.name,
                )
                continue
...
def get_mcp_manager() -> MCPManager:
    ...
    if _manager is None:
        ...
        from Mudabbir.lifecycle import register
