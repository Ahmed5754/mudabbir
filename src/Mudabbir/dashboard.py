"""Mudabbir Web Dashboard - API Server
...
try:
    ...
    raise ImportError(
        ...
        "but not installed. Reinstall with: pip install --upgrade Mudabbir"
    ) from _exc

from Mudabbir.agents.loop import AgentLoop
from Mudabbir.bootstrap import DefaultBootstrapProvider
from Mudabbir.bus import get_message_bus
from Mudabbir.bus.adapters.websocket_adapter import WebSocketAdapter
from Mudabbir.config import Settings, get_access_token, get_config_path, regenerate_token
...
from Mudabbir.security.rate_limiter import api_limiter, auth_limiter, cleanup_all, ws_limiter
from Mudabbir.security.session_tokens import create_session_token, verify_session_token
from Mudabbir.skills import SkillExecutor, get_skill_loader
from Mudabbir.skills.repo_sync import sync_skill_repositories
from Mudabbir.tunnel import get_tunnel_manager
...
# Create FastAPI app
app = FastAPI(title="Mudabbir Dashboard")
...
async def broadcast_reminder(reminder: dict):
    ...
    try:
        from Mudabbir.bus.notifier import notify
...
async def _start_channel_adapter(channel: str, settings: Settings | None = None) -> bool:
    ...
    if channel == "whatsapp":
        ...
        if mode == "personal":
            from Mudabbir.bus.adapters.neonize_adapter import NeonizeAdapter
    ...
    if channel == "webhook":
        from Mudabbir.bus.adapters.webhook_adapter import WebhookAdapter
...
async def startup_event():
    ...
    for ch in (
        ...
        try:
            if (
                ch == "whatsapp"
                and settings.whatsapp_mode == "personal"
                and not settings.whatsapp_personal_autostart
            ):
                logger.info("WhatsApp personal auto-start is disabled in settings.")
                continue
            if ch == "telegram" and not settings.telegram_autostart:
                logger.info("Telegram auto-start is disabled in settings.")
                continue
    ...
    try:
        from Mudabbir.deep_work import recover_interrupted_projects
    ...
    try:
        from Mudabbir.mcp.config import cleanup_invalid_mcp_configs
        from Mudabbir.mcp.manager import get_mcp_manager, set_ws_broadcast
        ...
        # Auto-clean stale/invalid configs (missing stdio runtime command, duplicates)
        cleanup_stats = cleanup_invalid_mcp_configs()
        if cleanup_stats.get("removed", 0):
            logger.info(
                "MCP startup cleanup removed %d stale config(s)",
...
async def shutdown_event():
    ...
    # Stop MCP servers
    try:
        from Mudabbir.mcp.manager import get_mcp_manager
...
async def list_mcp_presets():
    ...
    from Mudabbir.mcp.config import load_mcp_config
    from Mudabbir.mcp.presets import get_all_presets
...
async def install_mcp_preset(request: Request):
    ...
    from Mudabbir.mcp.manager import get_mcp_manager
    from Mudabbir.mcp.presets import get_preset, preset_to_config
    ...
    # Preflight for stdio presets: provide actionable error if command missing.
    if config.transport == "stdio" and config.command:
        if shutil.which(config.command) is None:
            install_hint = (
                "Install Node.js (includes npx)"
                if config.command.lower() == "npx"
                else "Install uv (pip install uv)"
                if config.command.lower() == "uvx"
                else f"Install '{config.command}' and ensure it is in PATH"
            )
            return JSONResponse(
                {
                    "error": f"Missing MCP runtime command '{config.command}'. {install_hint}.",
                },
                status_code=400,
            )
    ...
    # Persist configs only when connected, or when OAuth is used (may complete later).
    if connected or config.oauth:
        mgr.add_server_config(config)

    if not connected:
        status = mgr.get_server_status().get(config.name, {})
        error = status.get("error") or "Failed to start MCP server"
        # For non-OAuth failures, return error and avoid storing broken config.
        if not config.oauth:
            return JSONResponse({"error": error, "connected": False}, status_code=500)

    return {
        "status": "ok",
        "connected": connected,
        "error": "" if connected else (mgr.get_server_status().get(config.name, {}).get("error", "")),
        "tools": [{"name": t.name, "description": t.description} for t in tools],
    }
...
async def mcp_oauth_callback(
    ...
    from Mudabbir.mcp.manager import set_oauth_callback_result

    if error:
        import html

        safe_error = html.escape(error)
        safe_desc = html.escape(error_description or "OAuth provider denied authorization.")
        return HTMLResponse(
            "<html><body style='font-family: sans-serif;'>"
            "<h3>OAuth authorization failed</h3>"
            f"<p><strong>Provider error:</strong> {safe_error}</p>"
            f"<p><strong>Description:</strong> {safe_desc}</p>"
            "<p>This is an external provider/auth issue (not a Mudabbir crash). "
            "You can close this tab and retry authorization with valid account/org access.</p>"
            "</body></html>",
            status_code=400,
        )
...
@app.post("/api/skills/sync-repos")
async def sync_skill_repos(request: Request):
    """Sync multiple GitHub repositories into local skills (recursive SKILL.md discovery)."""
    from fastapi.responses import JSONResponse

    data = await request.json()
    raw_sources = data.get("sources")
    if raw_sources is None and data.get("source"):
        raw_sources = [data.get("source")]
    if not isinstance(raw_sources, list):
        return JSONResponse({"error": "sources must be a list of GitHub repos"}, status_code=400)
    ...
    for item in raw_sources:
        ...
        if any(ch in src for ch in (";", "|", "&")):
            return JSONResponse({"error": f"Invalid source format: {src}"}, status_code=400)
        sources.append(src)

    if not sources:
        return JSONResponse({"error": "No valid sources provided"}, status_code=400)
    ...
    if dest_raw:
        try:
            dest_path = Path(dest_raw).expanduser().resolve()
        except Exception:
            return JSONResponse({"error": "Invalid destination path"}, status_code=400)
        if not str(dest_path).startswith(str(Path.home().resolve())):
            return JSONResponse({"error": "Destination must be inside your home directory"}, status_code=400)

    try:
        report = await asyncio.to_thread(
            sync_skill_repositories,
            sources=sources,
        ...
        report["loaded_user_invocable"] = len([s for s in loaded.values() if s.user_invocable])
        return report
    except Exception as exc:
        logger.exception("Skill repository sync failed")
        return JSONResponse({"error": str(exc)}, status_code=500)
...
async def remove_skill(request: Request):
    ...
    for base in [Path.home() / ".agents" / "skills", Path.home() / ".Mudabbir" / "skills"]:
        skill_dir = base / name
...
async def webhook_inbound(
    ...
    from Mudabbir.bus.adapters.webhook_adapter import WebhookSlotConfig
...
_CHANNEL_CONFIG_KEYS: dict[str, dict[str, str]] = {
    ...
    "whatsapp": {
        ...
        "personal_autostart": "whatsapp_personal_autostart",
...
_CHANNEL_DEPS: dict[str, tuple[str, str, str]] = {
    "discord": ("discord.ext.commands", "discord.py", "mudabbir[discord]"),
    "slack": ("slack_bolt", "slack-bolt", "mudabbir[slack]"),
    "whatsapp": ("neonize", "neonize", "mudabbir[whatsapp-personal]"),
    "telegram": ("telegram.ext", "python-telegram-bot", "mudabbir[telegram]"),
    "matrix": ("nio", "matrix-nio", "mudabbir[matrix]"),
    "teams": ("botbuilder.core", "botbuilder-core", "mudabbir[teams]"),
    "google_chat": ("googleapiclient.discovery", "google-api-python-client", "mudabbir[gchat]"),
}
...
async def install_extras(request: Request):
    ...
    from Mudabbir.bus.adapters import auto_install
    ...
    adapter_modules = [
        k for k in sys.modules if k.startswith("Mudabbir.bus.adapters.")
    ]
...
async def get_channels_status():
    ...
    result["whatsapp"]["personal_autostart"] = settings.whatsapp_personal_autostart
    result["telegram"]["autostart"] = settings.telegram_autostart
    return result
...
async def oauth_authorize(service: str = Query("google_gmail")):
    ...
    from Mudabbir.integrations.oauth import OAuthManager
...
async def oauth_callback(
    ...
    try:
        from Mudabbir.integrations.oauth import OAuthManager
        from Mudabbir.integrations.token_store import TokenStore
        ...
        return HTMLResponse(
            ...
            "<p>Tokens saved. You can close this window and return to Mudabbir.</p>"
        )
...
async def index(request: Request):
    ...
    for dist_name in ("Mudabbir", "mudabbir"):
        try:
            app_version = get_version(dist_name)
            break
        except PackageNotFoundError:
            continue
...
async def auth_middleware(request: Request, call_next):
    ...
    if not is_valid:
        cookie_token = request.cookies.get("Mudabbir_session")
        if cookie_token:
...
async def cookie_logout():
    ...
    response = JSONResponse(content={"ok": True})
    response.delete_cookie(key="Mudabbir_session", path="/")
    return response
...
async def setup_telegram(request: Request):
    ...
    try:
        ...
        async def handle_pairing_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            await update.message.reply_text(
                "🎉 **Connected!**\n\nMudabbir is now paired with this device.\nYou can close the browser window now.",
                parse_mode="Markdown",
            )
...
async def websocket_endpoint(
    ...
    cookie_token = websocket.cookies.get("Mudabbir_session")
    ...
    if resume_session:
        ...
        if len(parts) == 2 and parts[0] == "websocket":
            ...
            session_file = (
                Path.home() / ".Mudabbir" / "memory" / "sessions" / f"{resume_session}.json"
            )
    ...
    try:
        while True:
            ...
            # Handle settings update
                ...
                from Mudabbir.config import get_settings as _get_settings
            ...
            # ==================== Plan Mode API ====================
                ...
                from Mudabbir.agents.plan_mode import get_plan_manager
...
async def export_session(id: str = "", format: str = "json"):
    ...
    filename = f"Mudabbir-session-{id[:20]}.{ext}"
    return Response(
        content=content,
...
async def run_security_audit_endpoint():
    ...
    from Mudabbir.security.audit_cli import (
        _check_audit_log,
...
async def run_self_audit_endpoint():
    ...
    from Mudabbir.daemon.self_audit import run_self_audit
...
async def handle_tool(websocket: WebSocket, tool: str, settings: Settings, data: dict):
    ...
    if tool == "status":
        ...
        from Mudabbir.tools.status import get_system_status
        ...
        from Mudabbir.tools.screenshot import take_screenshot
        ...
        from Mudabbir.tools.fetch import list_directory
...
async def handle_file_browse(
    ...
    """
    from Mudabbir.tools.fetch import is_safe_path
