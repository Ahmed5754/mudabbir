---
title: Web Dashboard
description: "Mudabbir's built-in web dashboard provides real-time streaming chat, session management, channel configuration, MCP server controls, and memory settings in a single interface."
section: Channels
ogType: article
...
The web dashboard is Mudabbir's default interface. It provides a real-time chat experience with session management, tool activity monitoring, and full configuration capabilities.
...
export MUDABBIR_DASHBOARD_HOST="0.0.0.0"
export MUDABBIR_DASHBOARD_PORT=8000
mudabbir
```
...
- **Toggle channels** — Start and stop channels without restarting Mudabbir
