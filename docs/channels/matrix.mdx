---
title: Matrix
description: "Connect Mudabbir to Matrix for decentralized, federated AI messaging. Uses matrix-nio async client with sync_forever, message edit support via m.replace, and room-based sessions."
section: Channels
ogType: article
...
Mudabbir connects to the [Matrix](https://matrix.org) protocol via `matrix-nio`. Matrix is an open, decentralized communication protocol ideal for self-hosting enthusiasts.
...
|---------|-------------|-------------|
| Homeserver | `MUDABBIR_MATRIX_HOMESERVER` | Matrix homeserver URL |
| User ID | `MUDABBIR_MATRIX_USER_ID` | Bot's Matrix user ID |
| Access token | `MUDABBIR_MATRIX_ACCESS_TOKEN` | Authentication token |
| Device ID | `MUDABBIR_MATRIX_DEVICE_ID` | Device ID (auto-generated) |
| Allowed rooms | `MUDABBIR_MATRIX_ALLOWED_ROOM_IDS` | Comma-separated room IDs |
| Display name | `MUDABBIR_MATRIX_DISPLAY_NAME` | Bot display name |
