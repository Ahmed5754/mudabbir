...
description: "Integrate Mem0 for semantic long-term memory with auto-learning: Mudabbir automatically extracts and stores facts from conversations, then retrieves relevant memories via vector similarity search."
section: Memory
ogType: article
...
Embedding dimensions must match between the model and the vector store collection. Mudabbir includes a built-in dimensions lookup and auto-detection for Ollama models:
...
For unknown Ollama models, Mudabbir queries the model to auto-detect dimensions.
