...
<Tabs items={["cURL", "JavaScript", "Python"]}>
  <Tab title="cURL">
```bash
curl -X POST http://localhost:8888/api/deep-work/projects/PROJECT_ID/approve \
  -H "Authorization: Bearer YOUR_TOKEN"
```
  </Tab>
  <Tab title="JavaScript">
```javascript
const res = await fetch(`http://localhost:8888/api/deep-work/projects/${projectId}/approve`, {
  method: 'POST',
...
res = httpx.post(
    f"http://localhost:8888/api/deep-work/projects/{project_id}/approve",
    headers={"Authorization": "Bearer YOUR_TOKEN"},
)
```
  </Tab>
