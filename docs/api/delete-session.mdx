...
description: "Permanently delete a Mudabbir chat session and all its associated messages. This action is irreversible and removes the session from the index and storage."
...
<Tabs items={["cURL", "JavaScript", "Python"]}>
  <Tab title="cURL">
    ```bash
    curl -X DELETE "http://localhost:8888/api/sessions/session_abc123" \
  ...
  <Tab title="JavaScript">
    ...
    const response = await fetch(`http://localhost:8888/api/sessions/${sessionId}`, {
      method: "DELETE",
