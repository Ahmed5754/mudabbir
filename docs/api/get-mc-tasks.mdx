...
description: "List all tasks managed by Mudabbir's Mission Control. Filter by status, assigned agent, or tags. Returns task details including priority, dependencies, project association, and execution state."
...
<Tabs items={["cURL"]}>
  <Tab title="cURL">
```bash
curl "http://localhost:8888/api/mission-control/tasks?status=in_progress" \
