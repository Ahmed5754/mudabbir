...
description: "List all AI agents registered in Mudabbir's Mission Control system. Filter by status (idle, active, blocked, offline) and view each agent's name, role, specialties, and last heartbeat."
...
<Tabs items={["cURL", "JavaScript", "Python"]}>
  <Tab title="cURL">
```bash
curl http://localhost:8888/api/mission-control/agents \
  -H "Authorization: Bearer YOUR_TOKEN"
```
  </Tab>
  <Tab title="JavaScript">
```javascript
const res = await fetch('http://localhost:8888/api/mission-control/agents', {
...
res = httpx.get(
    "http://localhost:8888/api/mission-control/agents",
    headers={"Authorization": "Bearer YOUR_TOKEN"},
)
data = res.json()
```
