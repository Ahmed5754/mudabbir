...
<Tabs items={["cURL", "JavaScript", "Python"]}>
  <Tab title="cURL">
    ```bash
    curl -X GET "http://localhost:8888/api/self-audit/reports/2024-01-15" \
      -H "Authorization: Bearer <token>"
    ```
  </Tab>
  <Tab title="JavaScript">
    ```javascript
    const response = await fetch("http://localhost:8888/api/self-audit/reports/2024-01-15", {
  ...
  <Tab title="Python">
    ...
    response = requests.get(
        "http://localhost:8888/api/self-audit/reports/2024-01-15",
        headers={"Authorization": "Bearer <token>"}
    )
    print(response.json())
    ```
