...
<ResponseField name="oauth" type="boolean">Whether this preset uses OAuth authentication</ResponseField>

<RequestExample>
<Tabs items={["cURL", "JavaScript", "Python"]}>
  <Tab title="cURL">
    ```bash
    curl -X GET "http://localhost:8888/api/mcp/presets" \
      -H "Authorization: Bearer <token>"
    ```
  </Tab>
  <Tab title="JavaScript">
    ```javascript
    const response = await fetch("http://localhost:8888/api/mcp/presets", {
  ...
  <Tab title="Python">
    ...
    response = requests.get(
        "http://localhost:8888/api/mcp/presets",
        headers={"Authorization": "Bearer <token>"}
    )
    print(response.json())
    ```
