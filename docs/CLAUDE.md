...
This is the documentation site for Mudabbir, a self-hosted AI agent. The docs are 120+ MDX files consumed by an Astro-based static site generator. The built output lives in `../docs-dist/` (Astro SSG with Pagefind search and Partytown).
...
├── introduction/              # 2 pages — welcome, why-mudabbir
...
- **Package name duality**: Internal Python package is `mudabbir`, public-facing name in docs is `Mudabbir`. Import paths use `mudabbir` (e.g., `from mudabbir.tools.registry import ...`).
...
- **Backend count**: 3 backends (Claude Agent SDK, Mudabbir Native, Open Interpreter).
