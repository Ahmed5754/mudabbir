...
description: "Mudabbir's two-tier prompt injection scanner combines fast regex heuristics with optional LLM analysis to detect and block injection attempts in user messages and tool outputs."
section: Security
ogType: article
...
The injection scanner protects Mudabbir from prompt injection attacks using a two-tier detection approach.
